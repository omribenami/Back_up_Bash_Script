...
# this bash script is in charge on your third week of month Google Drive backup
...
~/.google-drive-upload/bin/gupload --config /home/ben-ami/.googledrive.conf -o -r "$folder_id" "3rdwom_backup.tar.gz" # gupload upload command (-o stands for overwrite) 
echo "backup process is done" 
echo "$(date) - 3rd week of month backup has been uploaded to Gdrive successfully" >> ~/homeassistant_backups/backup.log # adds the date this process ran to the log file